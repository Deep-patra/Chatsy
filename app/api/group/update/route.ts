...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (name)
      obj.name = String(formdata.has('name'))

    if (description)
      obj.description = String(formdata.has('description'))
    ...
    if (photo) {
      ...
      const thumbnail_url = await storeFile(thumbnail, 'thumbnails', `${uuid}-thumhnail.png`)
    ...
    return new NextResponse(
      JSON.stringify({ result: 'ok', group: { group_id: groupRef.id, ...newDoc.data() } })
    )
