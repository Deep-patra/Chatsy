...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const groupRef = db.collection('groups').doc(String(formdata.get('group_id')))
    const groupDoc = await groupRef.get()

    if (groupDoc.get('admin_id') !== String(formdata.has('user_id')))
      throw new Error("User is not the admin of the group.")

    const userRef = db.collection('users').doc(String(formdata.get('member_id')))
    ...
    logger.error(e)
    return new NextResponse(JSON.stringify({ error: e.message || "Invalid Request!" }), { status: 400 })
  }
}
