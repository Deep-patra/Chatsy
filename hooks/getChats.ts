...
type UseGetChatsReturn = [
  IChat[],
  (contactId: string, message: IMessage[]) => void,
  (chats: IChat[]) => void
]
...
function addMessage(messages: IMessage[], message: IMessage): IMessage[] {
  ...
  if (message.time.getTime() < firstMessage.time.getTime())
    messages.unshift(message)
  else if (message.time.getTime() > lastMessage.time.getTime())
    messages.push(message)
  else {
    for (let idx = 0; idx < messages.length - 1; idx++) {
      if (
        messages[idx].time.getTime() > message.time.getTime() &&
        messages[idx + 1].time.getTime() < message.time.getTime()
      )
...
const useGetChats = (): UseGetChatsReturn => {
  ...
  const _changeChats = (_chats: IChat[]) => {
    changeChats(_chats)
  }
    ...
    updatedChats.forEach((value: IChat) => {
      if (value.userId === contactId) {
        messages.forEach((message) => {
          value.messages = [...addMessage(value.messages, message)]
        })
      }
    })
