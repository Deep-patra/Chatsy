...
const isAlreadyInInvites = (invites: I_Invites, searchUser: User): boolean => {
  ...
  found = found ?? invites.user.received.find((item) => item.from === searchUser.id)
...
export default function Search() {
  ...
  const checkSearchUser = useCallback((searchUser: User) => {
    if (!user) false

    if (user!.id === searchUser.id || isAlreadyInInvites(invites, searchUser) || isAlreadyInContacts(user!, searchUser))
      return true
    return false
  }, [user, invites])


  const handleSendInvite = useCallback((searchUser: User) => {
    if (!user)
      return
    ...
    if (value !== '')
      timeoutId = setTimeout(() => {
        ...
        User
          ...
          .catch(console.error)
          .finally(() => { changeLoadingResults(false) })
      }, 1000)
  ...
  return (
    <div className="flex flex-col gap-3 | h-full">
      ...
      <div className="w-full h-full flex-grow | overflow-y-auto | decorate-scrollbar">
              ...
              results.map((u, index) => (
                ...
                >
                  <div className="w-full | flex flex-row items-center gap-2">
                    ...
                    <span className="text-md text-white2 | group-hover:text-brightWhite">{u.name}</span>
                  </div>
        ...
        {/* no search results */}
        {(!loadingResults && results.length === 0 && value !== "") && <NoResults/>}
        ...
        {(value === "") && (
          <div className="w-full h-full flex-grow | flex flex-col items-center justify-center gap-1 | text-white1">
            ...
            <span className="text-xs text-white2"> search for an username </span>
          </div>
        )}
      </div>
    </div>
