...
interface IEditButtonProps {
  edit: boolean
  handleEdit: (changeLoading: (ns: boolean) => void) => void
}
...
export default function Settings() {
  ...
  const [enableNotifications, changeEnableNotifications] =
  ...
  const [editProfilePicture, changeEditProfilePicture] =
    useState<boolean>(false)
  ...
  const handleEditName = useCallback(
    (changeLoading: (ns: boolean) => void) => {
      if (editName && username !== user!.name) {
        changeLoading(true) // show the loading state
        user!
    ...
    (changeLoading: (ns: boolean) => void) => {
      if (editDescription && user!.description !== description) {
        ...
        user!
          ...
          .finally(() => {
            changeEditDescription(false)
            changeLoading(false)
          })
      } else changeEditDescription(true)
    },
    ...
    (changeLoading: (ns: boolean) => void) => {
      if (editProfilePicture && (file || photo !== getPhotoURL(user!.photo))) {
        ...
        user!
          ...
          .finally(() => {
            changeEditProfilePicture(false)
            changeLoading(false)
          })
      } else changeEditProfilePicture(true)
    },
  ...
  return (
    <div className="relative | flex flex-col gap-1 | w-full h-full">
      <span className="flex flex-row items-center gap-2 | p-2 | text-white1">
      ...
      <main className="w-full h-full | flex flex-col gap-3 | p-2 | overflow-y-auto | decorate-scrollbar">
        ...
        <div className="flex flex-col gap-3 | w-full | p-1 | border-t border-t-white3">
          ...
          <div className="flex flex-col gap-1 | w-full">
            <span className="block | w-full | text-sm text-white1">
              Username
            </span>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="ml-auto | flex flex-row items-start gap-1">
              ...
              <EditButton
                ...
                handleEdit={handleEditProfilePicture}
              />
            </div>
          </div>
        </div>
        ...
        <div className="w-full | flex flex-col gap-2 | p-1 | border-t border-t-gray2">
          <div className="flex flex-col justify-start | text-white1">
            ...
            <p className="text-xs | text-white3">
          ...
          <div className="flex flex-row items-center justify-between">
            <span className="text-white1">
              <p className="text-inherit text-sm">Enable notifications</p>
            </span>
