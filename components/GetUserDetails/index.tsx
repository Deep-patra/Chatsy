...
export default function GetUserDetails() {
  ...
  const handleNext = () => {
    if (currentPage === 1 && user) {
      ...
      user.update({ name, photo, description }) 
        .then(() => {
  ...
  return (
    ...
    >
      <div className="min-w-[200] max-w-[400px] min-h-[200px] flex flex-col gap-2 flex-grow | bg-black1 | rounded-md | p-2 | shadow-md ">
        <span className="text-lg text-brightGreen p-1">Before you get started.</span>
        ...
        >
          <AnimatePresence>
            <motion.div
              ...
              exit={{ x: -50, opacity: 0, position: 'absolute', top: 0, left: 0 }}
            >
              {currentPage === 0 && (
                <NameAndDescription
                  {...{
        ...
        <div className="w-full | flex flex-row items-center justify-between | p-2">
          ...
          >
            {loading && <div
              className="flex flex-row items-center justify-center | w-5 h-5"
            >
              <Loader color="white" />
            </div>
            }
