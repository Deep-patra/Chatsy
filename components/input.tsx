'use client'

import classNames from 'classnames'
import { InputHTMLAttributes, ButtonHTMLAttributes } from 'react'

interface SecondaryButton extends ButtonHTMLAttributes<HTMLButtonElement> {}

interface IInputProps extends InputHTMLAttributes<HTMLInputElement> {
  primaryIcon?: JSX.Element
  secondaryIcon?: JSX.Element
  secondaryBut?: SecondaryButton
}

export default function Input(props: IInputProps) {
  const { primaryIcon, secondaryIcon, secondaryBut, ...inputProps } = props

  return (
    <div className="relative w-full | flex-grow flex flex-row gap-2 items-center | border rounded-md border-solid border-white3 p-1 form-input text-white1 hover:border-white1">
      {/* PRIMARY ICON */}
      {primaryIcon ? primaryIcon : null}

      <input
        {...inputProps}
        className={classNames(props.className, 'w-full text-lg text-white1')}
      />
      {/* SECONDARY ICON */}
      {secondaryIcon ? (
        <button {...secondaryBut}>{secondaryIcon}</button>
      ) : null}
    </div>
  )
}
