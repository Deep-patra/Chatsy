import { useState, useContext, useMemo, useCallback, useEffect, memo } from 'react'
...
export default function AddMember() {
  ...
  const contactsNotMember = useMemo(() => {
    if (activeChat instanceof Group && contacts.length > 0) {
      ...
      const new_contacts = contacts.filter((contact) => {
        if (activeChat.memberIds.find((value) => value === contact.id))
          return
  ...
  const handleAdd = useCallback((contact: Contact) => {

    if (activeChat instanceof Group && user)
      activeChat.sendInvite(user.id, contact.id)
        .catch(console.error)
  ...
  useEffect(() => {
    ...
    return () => document.body.removeEventListener(events.open_add_member, handleAddMember)
  }, [])
    ...
    >
      <>
        ...
        <ul className="flex flex-col gap-1 | overflow-y-auto | decorate-scrollbar">
          {contactsNotMember.length > 0 && contactsNotMember.map((contact) => (
            <li key={contact.id} className="relative | p-1 | flex flex-row items-center gap-2 | group hover:bg-black2 | rounded-md | hover:px-3 | transition-all | duration-200">
              <Image
                src={getPhotoURL(contact.photo)}
                className="w-10 h-10 | rounded-full | border-2 border-white3 | group-hover:border-white1"
              />
              <span className="max-w-[70%] | overflow-hidden | text-ellipsis text-sm text-white2 | group-hover:text-white">{contact.name}</span>
              <button
                type="button"
