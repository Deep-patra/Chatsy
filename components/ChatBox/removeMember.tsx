...
export default function RemoveMember() {
  ...
  const members = useMemo(() => {
    ...
    if (activeChat instanceof Group) {
      members = activeChat.members.filter((value) =>
        value.id !== activeChat.admin ? value : undefined
      )
    }
          ...
          activeChat
            ...
            .then(() => {
              dispatchSnackbarEvent({
                type: 'success',
                text: `${member.name} is removed`,
              })
            })
            .catch((error) => {
              dispatchSnackbarEvent({
                type: 'error',
                text: `cannot remove ${member.name}`,
              })
              console.error(error)
            })
            .finally(() => {
        ...
        <ul className="flex flex-col gap-1">
          {members.length > 0 &&
            members.map((member) => (
              <li
                key={member.id}
                className="relative | flex flex-row items-center gap-2 | p-1 rounded-md | group | hover:bg-black2 hover:px-2 | transition-all | duration-200"
              >
                {/* member picture */}
                <Image
                ...
                {/* member name */}
                <span className="text-xs text-white | max-w-[70%] text-ellipsis">
                  {member.name}
                </span>
